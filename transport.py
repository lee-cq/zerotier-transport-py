...
def _get_another_stream(num) -> socket.socket:
    """
...
def _connect(host, port, num):
    ...
    while True:
        ...
        try:
            ...
            print('\r can not connect %s:%s!' % (host, port), end=f'{int(time.time() - st)} s')
...
def main():
    ...
    for i in [0, 1]:
        ...
        sl = s.split(':')
        if len(sl) == 3 and (sl[0] == 'l' or sl[0] == 'L'):  # l:host:port
            host, port = sl[1], int(sl[2])
            s = socket.socket()
            s.bind((host, port))
            s.close()
            t = threading.Thread(target=_server, args=(sl[1], int(sl[2]), i))
            tlist.append(t)
...
if __name__ == '__main__':
    #sys.argv.extend(['l:127.0.0.1:33890', 'c:10.1.1.4:3389'])
    main()
